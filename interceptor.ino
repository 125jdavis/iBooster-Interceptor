...
// --- State ---
enum class Mode { PASSTHROUGH, ACTIVE };
...
void setMode(Mode m) {
  mode = m;
  digitalWrite(PIN_SWT_SRC, m == Mode::ACTIVE ? HIGH : LOW);
}

// -------------------------------------------------------
// Serial command handler
// Commands (send as single character + Enter):
//   'p' or 'P' → force PASSTHROUGH, lock manual override
//   'a' or 'A' → force ACTIVE, lock manual override
//   'r' or 'R' → release manual override, resume auto zero-detect
// -------------------------------------------------------
void handleSerial() {
  while (Serial.available() > 0) {
    ...
    switch (c) {
      case 'p': case 'P':
        ...
        atRest = false;  // reset zero-detect state
...
void setup() {
  ...
  Serial.println("# Commands: P=passthrough  A=active  R=resume auto");
...
if (now - timerPrint >= PRINT_RATE) {
    ...
    Serial.println(mode == Mode::ACTIVE ? "ACTIVE" : "PASSTHRU");
    timerPrint = now;
  }
}
