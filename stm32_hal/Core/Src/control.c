...
{
    const float s2_travel = (S2_REST_DUTY_PCT - s2_pct) / (S2_REST_DUTY_PCT - S2_FULL_DUTY_PCT) * 100.0f; /* travel estimate derived from S2 duty */
    const float s4_travel = (s4_pct - S4_REST_DUTY_PCT) / (S4_FULL_DUTY_PCT - S4_REST_DUTY_PCT) * 100.0f; /* travel estimate derived from S4 duty */
...
{
    const float clamped = control_clamp_pct(travel_pct); /* travel input constrained to valid lookup range */

    for (uint32_t index = 0U; /* current calibration segment index */ index < (CAL_POINT_COUNT - 1U); ++index) {
        if (clamped <= CAL_IN_PCT[index + 1U]) {
            const float span = CAL_IN_PCT[index + 1U] - CAL_IN_PCT[index]; /* input width of the active curve segment */
            const float t = (span > 0.0f) ? ((clamped - CAL_IN_PCT[index]) / span) : 0.0f; /* interpolation factor within the segment */
...
{
    const float normalized = control_clamp_pct(travel_pct) / 100.0f; /* 0..1 travel fraction used for duty interpolation */
...
{
    const float max_step = COMMAND_RAMP_RATE_PCT_PER_SEC * dt_seconds; /* maximum travel change allowed this update */
