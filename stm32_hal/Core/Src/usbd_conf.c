...
uint8_t USBD_LL_IsStallEP(USBD_HandleTypeDef *pdev, uint8_t ep_addr)
{
    ...
    uint8_t ep_index = ep_addr & 0x7FU;
    return (ep_addr & 0x80U) ? hpcd->IN_ep[ep_index].is_stall : hpcd->OUT_ep[ep_index].is_stall;
}
...
void HAL_PCD_ResetCallback(PCD_HandleTypeDef *hpcd)
{
    ...
    switch (HAL_PCD_GetState(hpcd)) {
    case HAL_PCD_STATE_READY:
